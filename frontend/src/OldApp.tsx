...
import { TabGlobalLive } from './components/TabGlobalLive';
...
import { Map, Users, Cpu, Wrench, Shield, AlertCircle, X, Sliders, Globe } from 'lucide-react';

export const App: React.FC = () => {
  // ── Navigation & Role ─────────────────────────────────────────────────────
  const [activeTab, setActiveTab] = useLocalStorage<'global' | 'map' | 'roles' | 'cv' | 'dispatch' | 'pipeline' | 'control'>('dp-activeTab', 'global');
  ...
  // Emergency Nav Listener
  useEffect(() => {
    const handleEmergency = () => setActiveTab('map');
    window.addEventListener('emergency-nav', handleEmergency);
    return () => window.removeEventListener('emergency-nav', handleEmergency);
  }, [setActiveTab]);
  ...
  const tabs = [
    { id: 'global', label: 'Global Live Disasters', icon: Globe },
      ...
      <main className="max-w-[1600px] mx-auto w-full px-4 py-6 flex-grow flex flex-col gap-6">
        ...
        <div className="flex-1 flex flex-col gap-6">
          ...
          <div className="flex-1">
            {activeTab === 'global' && <TabGlobalLive />}
